...
type ConcessionFormProps = {
  onSuccess: () => void;
  renewApplicationId?: string;
};

export const ConcessionForm: React.FC<ConcessionFormProps> = ({ onSuccess, renewApplicationId }) => {
  const { user } = useAuth();
  ...
  // If renewing, prefill fields from the existing application
  useEffect(() => {
    ...
    (async () => {
      const { data, error } = await supabase
        ...
        .select('*')
        .eq('id', renewApplicationId)
        .single();

      if (error) {
        console.error('Error fetching application to renew:', error);
        return;
      }
      if (data) {
        setFormData({
          ...
          railwayType: data.railway_type ?? '',
          passType: data.pass_type ?? '',
          dateOfBirth: data.date_of_birth ?? '',
          concessionFormNo: '', // new number will be generated by admin or left blank
          age: data.age ? String(data.age) : '',
          previousPassDate: data.previous_pass_date ?? '',
          previousPassExpiry: data.previous_pass_expiry ?? '',
          seasonTicketNo: data.season_ticket_no ?? ''
        });
      }
    })();
  }, [renewApplicationId]);

  // ... keep your handleInputChange & handleFileChange as before (copy from your file)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // compute validity window based on chosen pass type
